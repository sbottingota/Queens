...
impl State {
    ...
    pub fn new() -> Self {
        ...
        while groups.iter().map(|group| group.len()).sum::<usize>() < GRID_SIZE*GRID_SIZE {
    ...
    // returns a tuple of states: (unsolved state, solution)
    pub fn new_unsolved() -> (Self, Self) {
        loop { // try random states until one has exactly one solution
            for new_state in State::new().solutions(N_PREPLACED_QUEENS) {
                let solutions: Vec<_> = new_state.solutions(GRID_SIZE).collect();

                if solutions.len() == 1 {
                    return (new_state, solutions[0].clone());
                }
            }
        }
    }
    ...
    pub fn solutions(&self, total_queens: usize) -> impl Iterator<Item=Self> {
        SolverIterator::new(self, total_queens)
    }

    pub fn can_add_queen(&self, x: usize, y: usize) -> bool {
